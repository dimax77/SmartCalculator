...
fun CircleScreen(circleViewModel: CircleViewModel = CircleViewModel()) {
    AppLogger.info("CircleScreen", "Screen opened")
    ...
    Column(
        ...
        Button(onClick = {
            AppLogger.info("CircleScreen", "\"Check circles\" Button pressed")
            try {
                ...
                AppLogger.debug("CircleScreen", "Circles check result: $result")
            } catch (e: Exception) {
                AppLogger.error("CircleScreen", "Error checking circles", e)
            }
        }) {
            Text("Check circles")
        }
